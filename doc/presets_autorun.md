...
[presets.'preset-3']
autorun = 'hostname == "workstation"' # Autorun only on specific host.

[presets.'preset-4']
autorun = 'linux && hostname == "laptop1"' # Autorun only if hostname is "laptop1".
...
- OS type: `linux` (always true on Linux; `windows`/`darwin` evaluate to false)
